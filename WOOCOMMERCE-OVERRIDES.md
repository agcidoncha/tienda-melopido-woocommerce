...
| [`checkout/form-pay.php`](wp-content/themes/bricks-child/woocommerce/checkout/form-pay.php) | `woocommerce/templates/checkout/form-pay.php` (v10.9.0) | Página real de "pagar un pedido pendiente" (vive dentro de `[woocommerce_checkout]` en la página "Finalizar compra", no existe como plantilla Bricks aparte — la plantilla Bricks "Recibo del pedido"/wc_order_receipt nunca se usa de verdad, ver [[project_mobile_review_checklist]]). Añade un título ("Completa el pago de tu pedido") y envuelve todo en `<div class="melopido-recibo-pedido">` para darle aspecto de tarjeta — ver `css/recibo-pedido.css`. Ningún hook/lógica de pago tocado. |
